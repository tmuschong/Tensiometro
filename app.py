...
def calcular_resumen(valores):
    ...
    return [
        max(valores),
        min(valores),
        round(statistics.mean(valores),1),
        round(statistics.stdev(valores),1) if len(valores)>1 else 0
    ]
...
def home():
    if request.method == "POST":
        ...
        if len(sistolica)>0:
            filas += f"<tr style='font-weight:bold; background-color:#f2f2f2;'><td>Promedio</td><td>{round(sum(sistolica)/len(sistolica),1)}</td><td>{round(sum(diastolica)/len(diastolica),1)}</td><td>{round(sum(ppm)/len(ppm),1)}</td><td>{round(sum(pp)/len(pp),1)}</td><td>{round(sum(dp)/len(dp),1)}</td><td>-</td><td>-</td></tr>"
...
def exportar_pdf():
    ...
    for i in range(len(sistolica)):
        tabla_datos.append([str(i+1), str(sistolica[i]), str(diastolica[i]), str(ppm[i]), str(pp[i]), str(dp[i]), f"{hora[i]:02d}:{minutos[i]:02d}", f"{dia[i]:02d}/{mes[i]:02d}/{ano[i]}"])
    tabla_datos.append(["Promedio", str(round(statistics.mean(sistolica),1)), str(round(statistics.mean(diastolica),1)),
                        str(round(statistics.mean(ppm),1)), str(round(statistics.mean(pp),1)), str(round(statistics.mean(dp),1)), "-","-"])
